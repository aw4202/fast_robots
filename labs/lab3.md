...
Wiring diagram, I2C address

###### Connecting without USB
Code needs to be uploaded over USB connection first then board can be detached and battery powered. To establish bluetooth connectivity in Jupyter at minimum uploaded code needs to begin BLE, add service and settings, advertise, and listen for central device. However print statements and data cannot be viewed in the Arduino environment so gatt characteristics must be set up and data sent to computer. 

###### I2C Scanner & Address
Running the scanner with a single ToF attached directly or via the breakout board returns the default address. Running the scanner with two ToFs returns conflicting results (all possible addresses)--since the sensors are default assigned the same address, I2C clashes.
...
###### Sensors in Parallel
Attached to breakout board, extra wire between artemis pin that's used for XSHUT and sensor that gets shut off. Address reassignment procedure as described in prelab (turn sensor 2 off, begin sensor 1, reassign 1address, turn 2 on & begin.) From the documentation I thought a sensor had to have its address changed, shut off then restarted for the change to take effect, and that a sensor had to begin first to be shut off, so getting the correct sequence of steps took several attempts. Also realizing XHSUT wasn't a sensor mode/internally programmed register but required connection to an Artemis pin.
