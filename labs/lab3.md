...
Short mode is less affected by ambient light and has faster sampling so better maps close environment. Given the blind spot this is an advantage. Long maps further away so may allow detecting obstacles sooner if within field of view and avoiding unneeded movement; however obstacles further away fall within a narrower range so are more likely to be in the blind spot and missed, at least are closer. Short mode was used for ToF characterization and likely in final car. <br>
The short ranging distance was found to be up to about 2.0 meters. The measurements were highly repeatable and accurate, with the same prediction function used for all of them and the average of the between the expected and predicted being 100mm or less across all 5 trials. The ranging time to consistently be around 52 mS regardless of distance then to drop to around 30 mS once the range was exceeded and sensor stopped working. <br>
<img src="https://raw.githubusercontent.com/aw4202/fast_robots/main/images/lab3/1.png" width="300" height="300" style="object-fit: fill;"> <br>
<img src="https://raw.githubusercontent.com/aw4202/fast_robots/main/images/lab3/2.png" width="300" height="300" style="object-fit: fill;"> <br>
<img src="https://raw.githubusercontent.com/aw4202/fast_robots/main/images/lab3/4.png" width="300" height="300" style="object-fit: fill;"> <br>
<img src="https://raw.githubusercontent.com/aw4202/fast_robots/main/images/lab3/5.png" width="300" height="600" style="object-fit: fill;"> <br>
...
<br> 
The loop running the program completes a measurement from a sensor then prints it on a new line so that the timestamp printed on each new line by Arduino is more precise. The same is repeated for the other sensor. <br>
...
##### 2 ToF + IMU
The IMU could be connected in series to the Artemis board and then to the breakout at its other port, or connected at the third QWIIC port. Its I2C address was different from the ToFs so there were no clashes. The IMU setup is the same as in lab 2 and the loop running the program is the same as above except with an extra measurement and print line for the IMU. <br>
...
###### I cover sensor 1 then rotate it to the nearby wall (a few hundred mm) followed by ceiling (2.9m). I repeat for sensor 2. Because I am using XSHUT only at the start of the program to change addresses the measurements are unaffected when the wire slips out. I pitch then roll the IMU by +/- 90 degrees.
